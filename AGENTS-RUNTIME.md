...
- **[gh](gh/SKILL.md)**: You must load this skill when working with `gh` command and its subcommands.
  GitHub CLI (`gh`) operations for issues, PRs, workflows, and API
- **[gh-api](gh-api/SKILL.md)**: You must load this skill when working with `gh api` command.
  GitHub CLI (`gh api`) operations for authenticated REST and GraphQL API requests
- **[gh-models](gh-models/SKILL.md)**: You must load this skill when working with `gh models` command.
  GitHub CLI models (`gh models`) operations for running and evaluating AI models
- **[gh-pr](gh-pr/SKILL.md)**: You must load this skill when working with `gh pr` command.
  GitHub CLI (`gh pr`) operations for creating, viewing, and managing pull requests
- **[gh-run](gh-run/SKILL.md)**: You must load this skill when working with `gh run` or `gh workflow` commands.
  GitHub CLI (`gh run` and `gh workflow`) operations for workflow runs, jobs, logs, and attempts
