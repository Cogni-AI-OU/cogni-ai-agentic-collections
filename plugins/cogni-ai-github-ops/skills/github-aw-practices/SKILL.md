...
- **Immediate Production Writes**: Giving a new, untested agentic workflow full write access to the main branch
  without going through a staged or shadow evaluation phase.
- **Unbounded Triggers**: Allowing any user to trigger expensive AI workflows via issue creation without implementing
  integrity filtering or safe-outputs.
- **Ignoring A/B Stats**: Running an A/B experiment but failing to actually collect or analyze the metrics needed
